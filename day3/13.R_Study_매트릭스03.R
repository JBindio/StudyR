...
m1 <- cbind(z, x) # 열기준 결합(cbind)
m1

y <- 36:41
y
m2 <- rbind(m1, y) # 행기준 결합(rbind)
m2
...
val1 <- tail(iris[,c('Sepal.Length', 'Petal.Length', 'Species')])
...
rownames(burger2) <- c('맥도날드', '롯데리아', '버거킹')
burger2
